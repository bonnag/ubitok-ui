...
import EventsPoller from "./events-poller.js";
let BigNumber = UbiTokTypes.BigNumber;

// lots of problems with watching for events in web3 -
// this takes a more poll-based approach ...
class PollingBridge {
  ...
  subscribeFutureMarketEvents = (callback) => {
    this.futureMarketEventsPoller.subscribe(
      (error, result) => {
        let decodedResult = UbiTokTypes.decodeMarketOrderEvent(result);
  ...
  subscribeFutureClientOrderEvents = (callback) => {
    this.futureClientEventsPoller.subscribe(
      (error, result) => {
        let decodedResult = UbiTokTypes.decodeClientOrderEvent(result);
        let ourAddress = this._getOurAddress();
        if (decodedResult.client.toLowerCase() === ourAddress.toLowerCase()) {
          callback(error, decodedResult);
        }
      }
    );
...
export { PollingBridge as default };
